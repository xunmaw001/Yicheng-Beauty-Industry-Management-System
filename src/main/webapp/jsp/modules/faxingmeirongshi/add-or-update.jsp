...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑发型美容师</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>发型美容师管理</span></li>
								<li class="breadcrumb-item active"><span>编辑发型美容师</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">发型美容师信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>技师类型</label>
										<select id="jishileixingSelect" name="jishileixing" class="form-control">
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>技师姓名</label>
										<input id="jishixingming" name="jishixingming" class="form-control" placeholder="技师姓名">
									</div>
									<div class="form-group col-md-6">
										<label>职位名称</label>
									...
									<div class="form-group col-md-6">
										<label>从业时间</label>
										<input id="congyeshijian" name="congyeshijian" class="form-control" placeholder="从业时间">
									</div>
									<div class="form-group col-md-6">
										<label>照片</label>
	...
	<script>
		...
		var tableName = "faxingmeirongshi";
		...
		var jishileixingOptions = [];

				function jishileixingSelect(){
					http("option/jishileixing/jishileixing","GET",{},(res)=>{
						if(res.code == 0){
							jishileixingOptions = res.data;
							var nullOption = document.createElement('option');
							var jishileixingSelect = document.getElementById('jishileixingSelect');
							jishileixingSelect.appendChild(nullOption);
							for(var i=0;i<jishileixingOptions.length;i++){
								var jishileixingOption = document.createElement('option');
								jishileixingOption.setAttribute('name','jishileixingOption');
								jishileixingOption.setAttribute('value',jishileixingOptions[i]);
								jishileixingOption.innerHTML = jishileixingOptions[i];									
								if(ruleForm.jishileixing == jishileixingOptions[i]){
									jishileixingOption.setAttribute('selected','selected');
								}
								jishileixingSelect.appendChild(jishileixingOption);
							}
						}
					});
				}	
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "faxingmeirongshi/info/" + id,
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("faxingmeirongshi/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("faxingmeirongshi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("faxingmeirongshi"+colName,"GET",{
		...
		function dataBind(){
			...
			var jishileixingOptions = document.getElementsByClassName("jishileixingOption");
			for(var jishileixingCount = 0; jishileixingCount < jishileixingOptions.length;jishileixingCount++){
				if(jishileixingOptions[jishileixingCount].getAttribute('value') == ruleForm.jishileixing){
					jishileixingOptions[jishileixingCount].setAttribute('selected','selected');
				}
			}
			$("#jishixingming").val(ruleForm.jishixingming);	
			$("#zhiweimingcheng").val(ruleForm.zhiweimingcheng);	
			$("#congyeshijian").val(ruleForm.congyeshijian);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.jishileixing != null && res.data.jishileixing != ''){

					var jishileixingOptions = document.getElementsByClassName("jishileixingOption");
						for(var jishileixingCount = 0; jishileixingCount < jishileixingOptions.length;jishileixingCount++){
							if(jishileixingOptions[jishileixingCount].getAttribute('value') == res.data.jishileixing){
								jishileixingOptions[jishileixingCount].setAttribute('selected','selected');
							}
						}
					}
					if(res.data != null && res.data != '' && res.data.jishixingming != null && res.data.jishixingming != ''){

					$("#jishixingming").val(res.data.jishixingming);
					}
					...
					if(res.data != null && res.data != '' && res.data.congyeshijian != null && res.data.congyeshijian != ''){

					$("#congyeshijian").val(res.data.congyeshijian);
					}
