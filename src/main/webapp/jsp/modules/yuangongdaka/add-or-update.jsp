		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑员工打卡</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>员工打卡管理</span></li>
								<li class="breadcrumb-item active"><span>编辑员工打卡</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">员工打卡信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>工号</label>
										<input id="gonghao" name="gonghao" class="form-control" placeholder="工号">
									</div>
									<div class="form-group col-md-6">
										<label>姓名</label>
									...
									<div class="form-group col-md-6">
										<label>考勤类型</label>
										<select id="kaoqinleixingSelect" name="kaoqinleixing" class="form-control">
												<option value=""></option>
												<option class="kaoqinleixingOption" value="上班">
													上班
												</option>
												<option class="kaoqinleixingOption" value="下班">
													下班
												</option>
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>打卡时间</label>
										<div id="dakashijianDate" class="input-append date form_datetime">
											<input id="dakashijian-input" name="dakashijian" size="20" type="text" readonly>
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "yuangongdaka/info/" + id,
		...
		function submit() {
			...
			if(validform() ==true && compare() == true){
				...
				httpJson("yuangongdaka/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("yuangongdaka/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#dakashijianDate").datetimepicker({ 
				autoclose: true,
		...
		function dataBind(){
			...
			var kaoqinleixingOptions = document.getElementsByClassName("kaoqinleixingOption");
			for(var kaoqinleixingCount = 0; kaoqinleixingCount < kaoqinleixingOptions.length;kaoqinleixingCount++){
				if(kaoqinleixingOptions[kaoqinleixingCount].getAttribute('value') == ruleForm.kaoqinleixing){
					kaoqinleixingOptions[kaoqinleixingCount].setAttribute('selected','selected');
				}
			}
			$("#dakashijian-input").val(ruleForm.dakashijian);
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.kaoqinleixing != null && res.data.kaoqinleixing != ''){

					var kaoqinleixingOptions = document.getElementsByClassName("kaoqinleixingOption");
						for(var kaoqinleixingCount = 0; kaoqinleixingCount < kaoqinleixingOptions.length;kaoqinleixingCount++){
							if(kaoqinleixingOptions[kaoqinleixingCount].getAttribute('value') == res.data.kaoqinleixing){
								kaoqinleixingOptions[kaoqinleixingCount].setAttribute('selected','selected');
							}
						}
					}
					if(res.data != null && res.data != '' && res.data.dakashijian != null && res.data.dakashijian != ''){

					$("#dakashijian-input").val(res.data.dakashijian);
					$('#dakashijianFileName').val(res.data.dakashijian)
					}
